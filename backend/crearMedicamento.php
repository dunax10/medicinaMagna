<?php
    ...
    if(isset($_SESSION['idUsuario']))
    {
        ...
        $sql = "SELECT nombre FROM medicamentos WHERE nombre = ? AND vigente = 1";
        ...
        {
            $stmt->close();
            $sql = "INSERT INTO `medicamentos`(`nombre`, `vigente`) VALUES (?,1)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('s', $nombre);
            $stmt->execute();
            $_SESSION['mensaje'] = "El medicamento fue creado con éxito";
            header("location: ../paginas/medicamentos/registrarMedicamentoVisual.php");
        }
        else
        {
