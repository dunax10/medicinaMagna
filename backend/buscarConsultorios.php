<?php
    ...
    if(!isset($_POST['searchTerm']) || empty($_POST['searchTerm'])) {
        ...
        $stmt = $conn->prepare("SELECT idConsultorio, nombre FROM consultorios ORDER BY nombre LIMIT ?");
        $stmt->bind_param('i', $numberofrecords);
    } else {
        $search = '%' . $_POST['searchTerm'] . '%'; // Search text con comodines
        // Mostrar resultados
        $stmt = $conn->prepare("SELECT idConsultorio, nombre FROM consultorios WHERE nombre LIKE ? ORDER BY nombre LIMIT ?");
