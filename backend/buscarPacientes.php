<?php
    ...
    if(!isset($_POST['searchTerm']) || empty($_POST['searchTerm'])) {
        ...
        $stmt = $conn->prepare("SELECT idPaciente, nombre FROM pacientes ORDER BY nombre LIMIT ?");
        $stmt->bind_param('i', $numberofrecords);
    } else {
        $search = '%' . $_POST['searchTerm'] . '%'; // Search text con comodines
        // Mostrar resultados
        $stmt = $conn->prepare("SELECT idPaciente, nombre FROM pacientes WHERE nombre LIKE ? ORDER BY nombre LIMIT ?");
